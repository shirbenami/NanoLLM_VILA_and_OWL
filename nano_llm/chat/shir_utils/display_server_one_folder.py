...
def _collect_items(root: Path, rel_root: Path) -> List[Item]:
    ...
    for img_path in root.glob("**/*"):
        ...
        items.append(Item(
            ...
            image_rel=str(img_path.relative_to(rel_root)),   # ← יחסית ל-ROOT תמיד
...
def create_app(root_dir: Path, scan_interval: float, latest_only: bool) -> Flask:
    app = Flask(__name__)
...
def main():
    ...
    app = create_app(root, args.scan_interval, args.latest_only)

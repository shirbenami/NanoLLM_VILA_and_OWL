...
def _append_entry_to_json(
    ...
    if forward_url:
        status, body = _http_post_json(forward_url, doc, timeout=6.0)
        if status == 200 or status == 201:
            cprint(f"[forward] posted JSON to {forward_url} (status {status})", "cyan")
        else:
            cprint(f"[forward][warn] failed to POST JSON to {forward_url} (status {status}): {body}", "yellow")

...
##### capture_frames from stream
capture_frames.py
**Purpose:**
This script captures frames from a live video stream (e.g., /dev/video0, RTSP, or file) and saves an image every few seconds or based on a predefined set of poses listed in /opt/missions/poses.json.
Each captured frame is saved with a matching .json file that includes both the 3D pose and an optional description generated by the VLM model.
...
4. Save images and JSON files (including the VLM-generated captions) under a timestamped folder inside captures/.
...
Example interactive mode (manual captures):
```

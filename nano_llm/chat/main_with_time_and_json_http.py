...
2) HTTP API server mode (--server) to accept image_path (and optional question)
   from external clients, run VILA, and append to the same JSON-by-image file.

Notes:
- Comments are in English only, as requested.
- This script reuses the same "single run cycle" logic (process_user_prompt)
  for both CLI and API, so behavior and JSON format remain consistent.
"""
...
# These imports assume your existing NanoLLM environment and utilities.
...
def process_user_prompt(user_prompt: str) -> str:
    """
    Execute one cycle:
    - Detect if prompt is an image path/URL and update last_image_path.
    - Append user prompt.
    - embed_chat + generate with the model.
    - Append bot reply.
    - Optionally save JSON bound to last_image_path.
    Returns the textual reply produced by the model.
    """
    ...
    # Save JSON per image if enabled
...
if args.server:
    ...
    def describe():
        ...
        Accepts JSON:
        {
          ...
          "question": "what is the color of the drone"  # optional
        }
        Runs two "cycles":
          1) image_path as the first prompt (image-level description)
          2) optional question as the second prompt
        Persists JSON-by-image according to current flags.
        """
        ...
        # First cycle: image_path as prompt (as in your logs)
        with _run_lock:
            resp1 = process_user_prompt(image_path)

        # Second cycle: optional question (context-aware follow-up)
        resp2 = None
        if question:
            with _run_lock:
                resp2 = process_user_prompt(question)

        return jsonify({
            "ok": True,
            "image_path": image_path,
            "question": question or None,
            "response_describe": resp1,
            "response_question": resp2
        })
